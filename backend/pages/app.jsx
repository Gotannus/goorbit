...
function AppContent() {
  ...
  const getLongToken = async () => {
    if (!fbToken || !fbAppId || !fbAppSecret) {
      setSyncErr("Preencha o token, App ID e App Secret para gerar token longo.");
      return;
    }
    setSyncLoad(true); setSyncErr(""); setSyncOk("");
    try {
      const res = await fetch(`/api/meta/longtoken?token=${encodeURIComponent(fbToken)}&app_id=${encodeURIComponent(fbAppId)}&app_secret=${encodeURIComponent(fbAppSecret)}`);
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setLongToken(data.long_lived_token);
      setFbToken(data.long_lived_token);
      setSyncOk(`✓ Token longo gerado! Válido por ~${data.expires_in_days} dias. Copie e salve!`);
      addLog("ok", `✓ Long-lived token gerado — válido por ${data.expires_in_days} dias`);
    } catch(e) {
        ...
        {tab==="sync"&&(
          <div className="main">
              ...
              {aiProvider === "gemini" ? (
                ...
                <>
                  ...
                  <div style={{display:"grid",gap:5,marginTop:8}}>
                    ...
                    <select className="finp" value={aiModel} onChange={e=>setAiModel(e.target.value)} style={{fontFamily:"var(--mono)",fontSize:12}}>
                      {["claude-3-5-sonnet-latest","claude-3-7-sonnet-latest","claude-3-haiku-20240307"].map(m=><option key={m} value={m}>{m}</option>)}
                    </select>
                  </div>
                </>
              )}
            ...
            <div className="card gb">
              ...
              <div style={{fontFamily:"var(--mono)",fontSize:11,color:"var(--txd)",marginBottom:10,lineHeight:1.7}}>
                Cole um token curto abaixo e clique em <span style={{color:"var(--gold)"}}>Gerar Token Longo (~60 dias)</span> para não precisar renovar toda hora.
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
                <input
                  className="finp"
                  type="password"
                  placeholder="Token de acesso (curto ou longo)…"
                  value={fbToken}
                  onChange={e=>{setFbToken(e.target.value);setSyncErr("");setSyncOk("");}}
                  style={{flex:1,fontFamily:"var(--mono)",fontSize:12}}
                />
              </div>
              {/* App ID + Secret para gerar long token */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div>
                  <div className="flbl" style={{marginBottom:4}}>App ID</div>
                  <input className="finp" placeholder="123456789" value={fbAppId} onChange={e=>setFbAppId(e.target.value)} style={{fontFamily:"var(--mono)",fontSize:12}}/>
                </div>
                <div>
                  <div className="flbl" style={{marginBottom:4}}>App Secret</div>
                  <input className="finp" type="password" placeholder="abc123..." value={fbAppSecret} onChange={e=>setFbAppSecret(e.target.value)} style={{fontFamily:"var(--mono)",fontSize:12}}/>
                </div>
              </div>
              <div className="brow" style={{marginBottom:8}}>
                <button className="btnol" onClick={getLongToken} disabled={syncLoad||!fbToken||!fbAppId||!fbAppSecret}>
                  🔄 Gerar Token Longo (~60 dias)
                </button>
              ...
              {longToken&&(
                <div style={{fontFamily:"var(--mono)",fontSize:10,background:"rgba(0,0,0,.3)",border:"1px solid var(--bdg)",borderRadius:7,padding:"8px 11px",color:"var(--gold)",wordBreak:"break-all",marginBottom:8}}>
                  <div style={{marginBottom:4,color:"var(--txd)"}}>Token longo (salve este):</div>
                  {longToken.slice(0,60)}…
                </div>
              )}
              {syncErr&&<div className="errbox">{syncErr}</div>}
              {syncOk&&<div className="succbox">{syncOk}</div>}
            </div>
